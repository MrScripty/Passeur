...
const client = await MuseClient.spawn({ museBin: "muse", args: ["serve", "--disable-write", "--disable-shell", "--sandbox-network", "restricted"], env, clientInfo: { name: "muse-bridge-probe", version: "0.1.0" }, onStderr: (chunk) => process.stderr.write(chunk) });
try {
