...
if (!profile.execution.implementation.enabled || (profile.execution.max_workers ?? 2) < 2) throw new Error("The profile must allow implementation and at least two workers");
...
try {
  ...
  const result = await client.callTool({ name: "delegate_to_muse_batch", arguments: { schema_version: 2, assignments } }, undefined, { timeout: profile.execution.task_timeout_ms + profile.execution.stop_grace_ms + 60_000 });
  console.log(JSON.stringify({ at: new Date().toISOString(), source: project, selected_agent: "muse", result }, null, 2));
