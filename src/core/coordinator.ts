...
import { changedFiles, createDiff, prepareWorkspace, type Workspace } from "../workspace/worktree.js";
...
export class Coordinator {
  #active: Promise<DelegateResult> | undefined;
  ...
  async delegate(request: DelegateRequest, context: RunContext): Promise<DelegateResult> {
    ...
    if (existing) {
      ...
      throw new BridgeError("TASK_ACTIVE", `Task ${existing.task_id} is active or was interrupted; it will not be launched again`);
    }
    ...
    if (this.#active) throw new BridgeError("TASK_ACTIVE", "This bridge permits one assignment at a time");
    ...
    const execution = this.#execute(stored, context);
    this.#active = execution;
    try { return await execution; } finally { this.#active = undefined; }
  }
  ...
  async #execute(stored: StoredRequest, context: RunContext): Promise<DelegateResult> {
    ...
    try {
      ...
      const stale = JSON.stringify(before) !== JSON.stringify(after) || (request.mode === "review" && JSON.stringify(initialStatus) !== JSON.stringify(finalStatus));
      const files = await changedFiles(workspace);
      result = { ...result, execution_status: run.status, worker_stop: run.worker_stop, summary: run.summary, model: { requested: this.profile.model, ...(run.reported_model ? { reported: run.reported_model } : {}) }, workspace: { ...result.workspace, stale }, changed_files: files, ...(run.error ? { error: run.error } : {}) };
  ...
  async #artifacts(taskId: string, workspace: Workspace, result: DelegateResult): Promise<void> {
    ...
    const manifest = { base_commit: workspace.base_commit, files: result.changed_files, generated_at: new Date().toISOString() };
...
export function compactResult(result: DelegateResult): DelegateResult {
  if (Buffer.byteLength(JSON.stringify(result)) <= 24_576) return result;
  return { ...result, summary: `${result.summary.slice(0, 4096)}\n[summary truncated; use muse_result]`, changed_files: result.changed_files.slice(0, 200), output_truncated: true };
}
