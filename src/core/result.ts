...
export function baseResult(taskId: string, request: DelegateRequest, profile: Profile, workspace?: Workspace): DelegateResult {
  return {
    schema_version: 2, task_id: taskId, request_key: request.request_key, execution_status: "failed", worker_stop: "not_started",
    worker_assessment: "unknown", summary: "", blockers: [], questions: [], model: { requested: profile.model },
...
export function compactResult(result: DelegateResult, budget = 16_384): DelegateResult {
  const compact = structuredClone(result);
  const bytes = () => Buffer.byteLength(JSON.stringify(compact));
  if (bytes() <= budget) return compact;
  compact.output_truncated = true;
  compact.summary = compact.summary.slice(0, 1024);
  if (compact.error) compact.error.message = compact.error.message.slice(0, 512);
  compact.blockers = compact.blockers.map((item) => item.slice(0, 512));
  compact.questions = compact.questions.map((item) => item.slice(0, 512));
  if (compact.delivery.reason) compact.delivery.reason = compact.delivery.reason.slice(0, 512);
  while (bytes() > budget) {
    // Optional evidence goes first; the unabridged result is retained on disk.
    const optional = [compact.changed_files, compact.checks, compact.artifacts, compact.delivery.commits ?? []].sort((a, b) => b.length - a.length)[0];
    if (optional?.length) { optional.pop(); continue; }
    if (compact.questions.length) { compact.questions.pop(); continue; }
    if (compact.blockers.length > 1) { compact.blockers.pop(); continue; }
    if (compact.summary.length) { compact.summary = compact.summary.slice(0, Math.max(0, compact.summary.length - 128)); continue; }
    break;
  }
  return compact;
}
export function resultReceipt(result: StoredResult, resource?: ResourceRecord): Record<string, unknown> {
  return {
    ...
    ...(result.schema_version === 2 ? { delivery: {
    ...
    resource_state: resource?.state ?? "legacy_unclassified", details: "muse_result", output_truncated: true,
  };
}
...
export function toolPayload(data: unknown, isError = false): { content: Array<{ type: "text"; text: string }>; isError: boolean } {
  ...
  if (Buffer.byteLength(JSON.stringify(payload)) > 24_576) throw new BridgeError("RESPONSE_TOO_LARGE", "Result retained; request a smaller range through muse_result");
  return payload;
}
/** Keep per-child identities even when several individually valid receipts exceed the aggregate limit. */
export function batchToolPayload(results: Array<Record<string, unknown>>, isError = false) {
  try { return toolPayload({ schema_version: 2, results }, isError); }
  catch (error) {
    ...
    const smaller = results.map((entry) => {
      ...
      const summary = {
        ...
        details: "muse_result", output_truncated: true,
      };
    ...
    return toolPayload({ schema_version: 2, output_truncated: true, results: smaller }, isError);
  }
}
