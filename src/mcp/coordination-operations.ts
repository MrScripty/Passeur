...
export const coordinationToolDescriptions: Readonly<Record<CoordinationToolName, string>> = Object.freeze({
  ...
  passeur_work: "Register this parent's external source worktree or enroll an existing implementation task controlled by this parent, change metadata readers, grant exact report/detail source access, set exact recipient source watches, or close a registration. Empty grant and watch lists revoke their respective authority. A watch can explicitly route an exact .js file as JSX or .h file as C/C++. Managed enrollment derives its prepared worktree/input from task records, with no restart or retasking. Does not announce/start a task. Registration verifies repository/input/workspace facts; sharing does not transfer control. Preserve operation keys for retry and use current revisions.",
...
export function decodeCoordinationToolArguments(tool: CoordinationToolName, value: unknown): CoordinationRequest {
  ...
  const allowed = tool === "passeur_coordination" ? request.kind !== "command"
    : tool === "passeur_work" ? kind === "register_external_work" || kind === "register_managed_work" || kind === "share_work" || kind === "grant_source" || kind === "watch_source" || kind === "close_work"
