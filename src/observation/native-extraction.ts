...
const typeTypes = new Set(["struct_item", "trait_item", "impl_item", "type_item", "enum_item",
...
export async function extractNativeFunctions(file: SourceFile, dialect: NativeDialect): Promise<Extraction> {
  ...
  if (dialect !== "rust" && dialect !== "typescript" && dialect !== "tsx") {
    throw new BridgeError("STRUCTURAL_DIALECT_UNSUPPORTED", "Function extraction is not qualified for this dialect");
  }
  ...
  const add = (outer: Parser.SyntaxNode, node: Parser.SyntaxNode, enclosing: string[], bindingName?: Parser.SyntaxNode,
    ...
    const name = nameNode ? implTrait ? `${implTrait.text} for ${nameNode.text}` : nameNode.text : null;
