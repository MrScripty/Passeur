...
import { parseWorkerMessage, finalReport } from '../../.passeur-core/src/agents/report.js';
...
test('worker commits invoke normal hooks; Passeur runs no application test gate', async t=>{
  ...
  const result=await c.execute(f.implementation('component'),context());
...
test('rejecting hook is not bypassed or rerun by Passeur',async t=>{
  ...
  const result=await c.execute(f.implementation('reject'),context());
...
test('uncommitted output is incomplete even when worker claims success',async t=>{
  ...
  const result=await c.execute(f.implementation('missing-commit'),context());assert.equal(result.execution_status,'completed');assert.equal(result.delivery.status,'incomplete');
});
test('explicit no-change explanation plus unchanged Git facts is sufficient',async t=>{
  ...
  const result=await c.execute(f.implementation('noop'),context());assert.equal(result.delivery.status,'no_changes_needed');assert.equal(result.delivery.head_commit,f.base);
});
test('no explanation and no commit is incomplete',async t=>{
  const f=await fixture(t),c=f.coordinator({run:async()=>done()});assert.equal((await c.execute(f.implementation('no-explanation'),context())).delivery.status,'incomplete');
});
test('failed run can retain real commits without becoming successful',async t=>{
  ...
  const result=await c.execute(f.implementation('partial'),context());assert.equal(result.delivery.status,'committed');assert.equal(result.execution_status,'failed');
});
test('full multi-commit range is reported from Git',async t=>{
  ...
  const result=await c.execute(f.implementation('multi'),context());assert.equal(result.delivery.commits.length,2);assert.equal(result.delivery.commits.at(-1),result.delivery.head_commit);
});
test('referenced deletion still produces complete manifest and applicable patch',async t=>{
  ...
  const result=await c.execute(f.implementation('delete',{context_files:['watched.txt']}),context());
...
test('rename scope warning includes the old path',async t=>{
  ...
  const result=await c.execute(f.implementation('rename',{allowed_paths:['allowed']}),context());assert.match(result.blockers.join('\n'),/watched.txt/);assert.deepEqual(result.changed_files,['allowed/new.txt']);
});
test('quoted, arrow and newline filenames survive Git export',async t=>{
  ...
  const result=await c.execute(f.implementation('odd-path'),context());assert.equal(result.delivery.status,'committed');assert.deepEqual(result.changed_files,[name]);
});
test('genuine finalization failure preserves an already observed committed delivery',async t=>{
  ...
  const result=await c.execute(f.implementation('finalize-fails'),context());assert.equal(result.worker_stop,'confirmed');assert.equal(result.error.code,'FINALIZATION_FAILED');assert.equal(result.delivery.status,'committed');
});
...
test('reported checks remain worker claims and nonfinite exit codes stay unknown',()=>{
  const parsed=finalReport(parseWorkerMessage('PASSEUR_MESSAGE {"schema_version":2,"kind":"final","summary":"done","assessment":"met","blockers":[],"questions":[],"checks":[{"command":"unit","cwd":"/work","exit_code":0}],"no_changes_reason":"already there"}'));
  assert.equal(parsed.checks[0].evidence,'worker_reported');assert.equal(parsed.no_changes_reason,'already there');assert.throws(()=>parseWorkerMessage('not json'),{code:'WORKER_MESSAGE_INVALID'});
});
test('aggregate receipts stay below the actual MCP payload limit',async t=>{
  ...
  const result=await c.execute(f.request('big'),context());const payload=toolPayload({results:Array.from({length:8},()=>resultReceipt(result))});assert.ok(Buffer.byteLength(JSON.stringify(payload))<=24576);
});
