...
const definition = (run, model) => ({ configure: () => ({ worker: { run }, modes: ['review', 'implement'], contract: 'fixture/1', requested_model: model, configuration: { model } }) });
async function fixture(t) {
  ...
  const policy = { implementation: { enabled: true, worktree_root: join(root, 'worktrees') }, task_timeout_ms: 60000, stop_grace_ms: 1000, max_workers: 2, max_queued_tasks: 8 };
  const profile = { schema_version: 2, execution: policy, agents: ['one', 'two'].map((id) => ({ agent_id: id, adapter_id: id, enabled: true, description: id, options: {} })) };
  const store = new MemoryStore(join(root, 'state'));
  const coordinators = [];
  const coordinator = (definitions, selected = profile) => {
    const c = new Coordinator(project, 'repo', selected.execution, store, new AgentRegistry(selected, definitions)); coordinators.push(c); return c;
  };
  ...
  return { root, project, git, base, policy, profile, store, coordinator, request };
}
function runtimeFor(t, f, definitions, loader = async () => f.profile) {
  ...
  const runtime = new RepositoryRuntime({ project: f.project, profilePath: join(f.root, 'profile.json') }, identity, {
    resolveBinding: async () => ({ project: f.project, repositoryId: 'repo', commonDir: join(f.project, '.git'), stateRoot: f.store.root, storeRoot: f.store.root, profilePath: join(f.root, 'profile.json') }),
...
test('registry isolates unsupported entries and only configured modes can be selected', async (t) => {
  ...
  assert.throws(() => { selected.snapshot.policy.task_timeout_ms = 1; }, TypeError);
});
test('mixed agents share capacity and produce independently observed Git commits', async (t) => {
  ...
  const pending = c.delegateBatch([f.request('a', 'one', 'implement'), f.request('b', 'two', 'implement')], context());
  try {
...
test('changed agent under an existing key conflicts; equivalent retry ignores current registry', async (t) => {
  ...
  const result = await c.delegate(f.request('same'), context());
  await c.shutdown();
  const empty = f.coordinator({}, { ...f.profile, agents: [] });
  const retained = await empty.delegate(f.request('same'), { ...context(), approvalAvailable: false });
  assert.equal(retained.task_id, result.task_id); assert.equal(starts, 1);
  await assert.rejects(empty.delegate(f.request('same', 'two'), context()), { code: 'REQUEST_KEY_CONFLICT' });
});
test('a duplicate subscriber detaches without cancelling the owner', async (t) => {
  ...
  const owner = c.delegate(f.request('same'), context());
  try {
    await started.promise;
    const cancel = new AbortController(), subscriber = c.delegate(f.request('same'), context(cancel));
...
test('mutating the caller assignment after admission cannot rewrite execution identity', async (t) => {
  ...
  const assignment = f.request('fixed'), pending = c.delegate(assignment, context()); assignment.objective = 'changed';
...
test('an unavailable agent does not start or invalidate a healthy registration', async (t) => {
  ...
  const rows = await c.delegateBatch([f.request('bad', 'two'), f.request('good')], context());
...
test('unconfirmed stop freezes future agents instead of silently routing around uncertainty', async (t) => {
  ...
  const result = await c.delegate(f.request('uncertain'), context());
  assert.equal(result.worker_stop, 'unconfirmed');
  await assert.rejects(c.delegate(f.request('replacement', 'two'), context()), { code: 'PROJECT_NEEDS_RECONCILIATION' }); assert.equal(other, 0);
});
test('retained result retrieval bypasses missing profile, approval, adapter and lease', async (t) => {
  ...
  const previous = await c.delegate(f.request('retained'), context()); await c.shutdown();
  const host = runtimeFor(t, f, {}, async () => { throw Error('profile must not load'); }); host.runtime.observeApproval(false);
  assert.equal((await host.runtime.delegate(f.request('retained'), context())).task_id, previous.task_id);
...
test('failed pre-admission config can be repaired without poisoning the diagnostic runtime', async (t) => {
  ...
  await assert.rejects(host.runtime.delegate(f.request('first'), context()), { code: 'PROFILE_INVALID' });
  assert.equal(host.runtime.status().execution.profile, 'blocked');
  assert.equal((await host.runtime.delegate(f.request('retry'), context())).execution_status, 'completed');
...
test('agent discovery and preparation neither start a worker nor fix a provisional catalog', async (t) => {
  ...
  await host.runtime.delegate(f.request('fix'), context());
