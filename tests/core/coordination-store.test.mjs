...
test('corrupt, unsupported and foreign-repository states remain distinguishable and untouched', async t => {
  ...
  const future = JSON.parse(original); future.schema_version = 3; await writeFile(f.file, JSON.stringify(future));
...
test('complete decoding distinguishes unsupported operation from malformed data', () => {
  ...
  assert.throws(() => decodeControl({ schema_version: 3 }, repo), { code: 'COORDINATION_VERSION_UNSUPPORTED' });
