...
Review mode records project dirtiness and digests explicitly referenced context files before and after execution. This is drift detection, not an immutable snapshot. Implementation mode requires a clean source checkout and an exact commit ID, creates a retained worktree and branch, and returns manifest and diff artifacts without integrating them.
