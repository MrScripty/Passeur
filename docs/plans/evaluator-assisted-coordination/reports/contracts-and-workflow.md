...
This report refines the structural/control contracts D1–D12 of [the plan](../plan.md). These are prospective contracts, not generated production schemas. Implement the schemas once in the named contract owners and derive public/IPC projections; do not maintain a parallel handwritten API definition here after acceptance.
...
| Interaction | Apply deterministic relevance/sharing rules plus separately qualified optional model advice | Interpret evidence and advice; coordinate with another parent |
...
Declared areas produce deterministic overlap. A separate evaluator may classify relationships between explicitly supplied intents under [the evaluator contract](evaluator-design.md); that advisory result is not a fact established by this structural path. Runtime routing uses exact file/declaration watches and direct observed overlap. Lexical call/import occurrences may be queried as unresolved syntax hints but are not resolved-call edges or default urgency signals. No graph traversal predicts downstream behavioral impact.
...
Notes are optional `intent`, `question`, `statement`, `agreement_proposal`, `acknowledgment`, or `resolution_update` records. Agreement status follows exact named acknowledgments; silence does not count. A declaration watch can report concrete changes against a named agreement. Optional agreement-text evaluation follows its separately qualified question and produces a hypothesis only; agreement status still requires exact acknowledgments. Revocation/closure affects future access and actions without rewriting already retained statement history.
...
## 11. Evaluator boundary

The [evaluator contract](evaluator-design.md) owns probabilistic judgments and their limited policy effects. It consumes immutable structural records through an authorized context projection, never mutable parser handles or implicit workspace access. Structural collection remains callable and useful without a model. Deterministic watch notices are neither delayed by pending scores nor suppressed by low scores.

`passeur_evaluation` supplies named authorized work/report references for bounded assessment or retrieves a retained result/cursor. Ordinary parents may not supply arbitrary model URLs, question text, scoring thresholds, tools or keys. Operator CLI configuration owns enablement and the fixed rubric policy. Existing submit/wait/change responses may include a separately versioned advice reference; strict historical responses remain unchanged. No new coding-agent API or model-generated message format is required.
