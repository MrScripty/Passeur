...
These are procedures, not executed results. SC01–SC17 remain pending; EA01–EA10 are specified separately in [evaluator verification](evaluator-verification.md). The named evidence kind describes the actual boundary traversed; a test directory or fixture name does not establish fidelity. Required-real host/provider checks remain distinct from controlled peers.
...
Assert the policy-selected recipient and exact bounded structural payload, not a prose semantic conclusion. With evaluation disabled, observe provider/evaluator launch/invocation boundaries: source monitoring adds **zero** inference requests, explanation requests, automatic worker messages and automatic model wake-ups. With evaluation enabled, EV04/EV08 separately account for every permitted decision-model request; the structural owner still makes none. Existing parent-requested worker execution is excluded from that zero count and recorded separately. Authorization is evaluated again before delivering details. Reading/acknowledgment/retry must not discard an undelivered notice or force every parent to fetch the whole board.
...
Prove bounded behavior at limits and zero calls from the structural path. EV09 measures the additional explicitly authorized evaluator calls and their total cost separately. Model context savings are reported as bytes/observations delivered versus the corresponding available raw source difference, with query/detail costs included. Cache hit rate and repeated-suppression counts explain results; no unmeasured universal efficiency claim is accepted.
