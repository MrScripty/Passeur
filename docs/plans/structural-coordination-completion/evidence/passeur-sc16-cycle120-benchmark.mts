...
async function publicBenchmark(): Promise<void> {
  ...
  if (!runtimeRoot || !isAbsolute(runtimeRoot) ||
      ...
      !Number.isSafeInteger(edits) || edits < 1 || edits > 120 ||
    ...
    throw new Error("Usage: npx tsx scripts/benchmark-structural.ts public --runtime-root ABSOLUTE_PATH [--installed] [--rounds 3..100] [--edits 1..120]");
  }
  ...
  const raw: Record<string, unknown> = { observed_at: new Date().toISOString(), runtime: identity,
    ...
    command: "npx tsx scripts/benchmark-structural.ts public --runtime-root ABSOLUTE_PATH [--installed] [--rounds 3..100] [--edits 1..120]",
    rounds, edits,
  ...
  try {
    ...
    raw.enabled = { monitor: "on", generation: enabled.generation,
      samples: await measurePublicProcess(enabled.pid, async () => {
        ...
        // Disposable SC16 supplement: alternate observed edits across three real grammars.
        const cyclePaths = ["project/repository-runtime.ts", "rust/sample.rs", "svelte/sample.svelte"];
        const editSamples = [];
        for (let editIndex = 0; editIndex < edits; editIndex++) {
          const path = cyclePaths[editIndex % cyclePaths.length]!;
          const before = await enabled.frontend.call("structural_current", {});
          const initial = before.reports.find((item: { path: string }) => item.path === path);
          const suffix = path.endsWith(".svelte")
            ? `\n<!-- SC16 grammar cycle ${editIndex} -->\n`
            : `\n// SC16 grammar cycle ${editIndex}\n`;
          const changed = Buffer.concat([changedBytes.get(path)!, Buffer.from(suffix)]);
          const sample = await measurePublicProcess(enabled.pid, async () => {
            const editStarted = performance.now();
            await writeFile(join(project, path), changed);
            let observed: any;
            const deadline = performance.now() + 10_000;
            while (performance.now() < deadline) {
              const current = await enabled.frontend.call("structural_current", {});
              observed = current.reports.find((item: { path: string }) => item.path === path);
              if (observed && observed.id !== initial?.id) break;
              await delay(50);
            }
            const status = observed && observed.id !== initial?.id ? "observed" : "unavailable";
            const editToCurrent = performance.now() - editStarted;
            const reportStarted = performance.now();
            const report = await enabled.frontend.call("structural_report", { work_id: ids[0]! });
            const reportLatency = performance.now() - reportStarted;
            const controlStarted = performance.now();
            await Promise.all([enabled.frontend.call("status", {}), enabled.frontend.call("tasks", { schema_version: 1, offset: 0, limit: 8 })]);
            const controlLatency = performance.now() - controlStarted;
            return { status, path, edit_to_current_ms: editToCurrent,
              baseline_artifact_id: initial?.id ?? null, observed_artifact_id: observed?.id ?? null,
              report_latency_ms: reportLatency, control_latency_ms: controlLatency,
              response_bytes: Buffer.byteLength(JSON.stringify(report), "utf8"),
              report_paths: report.reports.map((entry: { path: string }) => entry.path) };
          });
          editSamples.push({ ...sample.result, sampled_parent_cpu_ms: sample.parent_cpu_ms_sampled,
            sampled_child_cpu_ms: sample.child_cpu_ms_sampled,
            peak_parent_rss_bytes: sample.peak_parent_rss_bytes,
            peak_child_rss_bytes: sample.peak_child_rss_bytes });
          if (sample.result.status !== "observed") break;
        }
        const edit = { samples: editSamples, all_observed: editSamples.length === edits && editSamples.every(row => row.status === "observed"),
          edit_percentiles_ms: latencyPercentiles(editSamples.map(row => row.edit_to_current_ms)),
          report_percentiles_ms: latencyPercentiles(editSamples.map(row => row.report_latency_ms)),
          control_percentiles_ms: latencyPercentiles(editSamples.map(row => row.control_latency_ms)),
          context_bytes: editSamples.reduce((sum, row) => sum + row.response_bytes, 0),
          limitation: "Per-round child CPU/RSS sampling is a lower bound; each report reparses four paths across TypeScript, Rust and Svelte" };
        for (const path of cyclePaths) await writeFile(join(project, path), changedBytes.get(path)!);
