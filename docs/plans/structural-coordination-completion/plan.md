...
| Field | Value |
|---|---|
| Plan status | **Accepted** — final product source `28c2b5c7cdc52cb4e9456608843f7398d3c0eb95` and clean installed build `9d64524b32966919479153f2e9feff9e1f3bec3b59054bb28f6f94171e92d8de` passed objective verification and independent review |
| Acceptance status | **SC01–SC17 passed** on exact native/public/installed and live evidence, with documented process and measurement limits; [claim status](reports/claim-status.md) and [resource inventory](reports/final-resource-inventory.md) are authoritative |
| Selected design | Plan 2A, code-only structural coordination |
| Current phase | R6 objective verification complete |
| Exactly one next integration slice | **None** — this completion plan is accepted |
| Canonical path | `docs/plans/structural-coordination-completion/plan.md` |
| Plan operation | **start** at R0-S1; **continue** while Active; **verify** after source reached Implemented, now complete |
...
|---|---|
| **R0-S1 — Receiving-environment admission (Accepted)** | Adopt this explicit path; preserve F9 and supersede only the old remaining planning authority. Establish full checkout, exact tool/dependency satisfaction, native-build and test authority, real-host permissions, review ownership, and lead-owned SC16 resource design. Run baseline pinned checks and record actual modern support targets. The [R0 report](reports/receiving-environment.md) records the receiving machine and decisions; developer-selected limits and representative measurements are R4/R6 engineering evidence. |
| **R1-S1 — Real native vertical report (Accepted)** | Resolve and pin the actual native binding and all thirteen grammar candidates; prove build/load viability across the requested set early. Implement complete Rust/TypeScript capture → helper → extraction → report → authenticated CLI/MCP retrieval using existing registered/enrolled work. Include report/detail authorization and disposable-helper lifecycle. No fabricated extraction provider in production. Gate includes real native parsing, Unicode ranges, common/different bases, add/add, body-only, errors and cancellation. |
| **R2-S1 — All required language extraction (Accepted)** | Complete every L01–L13 row, embedding, default masking and conservative matching with independently reviewed expected output. Package grammars and queries through the same native contract. A partial language milestone is not full acceptance. |
| **R3-S1 — Automatic announcement/submission linkage (Accepted)** | Add versioned inline/reference submission, advisory/gated preflight, durable binding and prepared-workspace attachment. Exercise actual TaskStore and coordinator admission, simultaneous parents, source-view differences, publication failure, recovery and ownership races. No extra native run on a retry. |
| **R4-S1 — Live observation and quiet delivery (Accepted)** | Integrate real watchers, helper jobs, caches, generation-safe snapshots, per-recipient notices/cursors and current-source authorization. Demonstrate public retrieval, event loss, replaced files, delayed results, disconnected parents, no-op edit storms, saturation and zero observation-origin model/build calls. |
| **R5-S1 — Native release artifact and migration (Accepted)** | Clean source `4ab24e0` and installed build `2727ed8d…` first passed the all-language offline native/public probe and gated installed tests; final build `9d64524b…` passed the replacement gates. The disposable old-to-new running-service handover returned a typed build conflict, drained explicitly and reopened the same root. Its actual handover limit is zero active workers; active-worker continuation was verified separately under SC12/SC15. |
| **R6-S1 — Objective acceptance and external review (Accepted)** | SC01–SC17 passed on the final material candidate with the real two-parent/three-worker workflow, installed host/native adapters, representative measurements, independent review, affected re-verification and [resource accounting](reports/final-resource-inventory.md). Next slice is none. |
...
Retain the original **SC01–SC17 identifiers** so prior evidence can be traced. Final claim status is in [the claim ledger](reports/claim-status.md); source, installed and live evidence now satisfy objective acceptance on the stated scope and limits. Baseline results remain references, not substituted final acceptance. Exact procedures, path boundaries and evidence requirements are in [verification and release](reports/verification-and-release.md).
...
Final delivery includes source/tests, reproducible native build inputs, supported-version matrix, usable skills/runbooks, updated evidence/issue records and the reviewed integration candidate. The current authority table and linked records above form the compact decision/evidence index; this file retains the original contract for traceability. Do not certify unrelated or future codebase compliance, and do not close unrelated pending shared-service claims by association.
